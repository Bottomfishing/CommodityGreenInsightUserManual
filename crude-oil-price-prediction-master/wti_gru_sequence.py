...
def main() -> None:
    ...
    if (not args.no_plot) or args.vmd_plot:
        ...
        if n_cmp > 0:
            ...
            axes_pre[0].set_title("Before preprocessing: raw returns")
            axes_pre[1].plot(t_cmp, training_set[:n_cmp].ravel(), color="tab:green", linewidth=0.9)
            axes_pre[1].set_title("After preprocessing: returns used for training (VMD/alignment)")
            axes_pre[2].plot(t_cmp, training_set_scaled[:n_cmp].ravel(), color="tab:orange", linewidth=0.9)
            axes_pre[2].set_title("After preprocessing: standardized returns (scaler output)")
            axes_pre[2].set_xlabel("Sample index")
            ...
            ax_hist.set_title("Distribution before vs after preprocessing (returns)")
        ...
        if feat_mat is not None and feat_scaled is not None and feat_mat.shape[1] > 0:
            ...
            ax_fx.set_title("Feature before vs after scaling (feature #1)")
            ax_fx.set_xlabel("Sample index")
            ax_fx.set_ylabel("Value")
        ...
        if target_original is not None and training_set is not None:
            ...
            ax_vmd_ba.plot(
                ...
                label="After VMD (reconstructed returns)",
                color="tab:green",
                linewidth=0.9,
                alpha=0.9,
            )
            ax_vmd_ba.set_title("Label series before vs after VMD")
            ax_vmd_ba.set_xlabel("Sample index")
            ax_vmd_ba.set_ylabel("Return")
        ...
        if args.vmd_plot and vmd_imfs is not None and target_original is not None:
            ...
            axes_vmd[0].set_title("VMD decomposition: original label series (full range)")
            for i in range(n_imf):
                axes_vmd[i + 1].plot(t_imf, vmd_imfs[i], linewidth=0.9)
                axes_vmd[i + 1].set_title(f"IMF {i + 1} (prefix length {len(t_imf)})")
            axes_vmd[-1].plot(t_v, training_set.ravel(), color="tab:green", linewidth=0.9)
            axes_vmd[-1].set_title("VMD reconstructed label (used for training)")
            axes_vmd[-1].set_xlabel("Sample index")
        ...
        t = np.arange(len(y))
        label_unit = "Return"
        ax_y.plot(t, y.ravel(), label=f"True {label_unit}", color="tab:blue", linewidth=1.1)
        ax_y.plot(
            t,
            p.ravel(),
            label=f"Predicted {label_unit}",
            color="tab:orange",
            linewidth=1.0,
            alpha=0.92,
        )
        ax_y.set_title("Test set: true vs predicted (horizon=1, inverse-transformed)")
        ax_y.set_xlabel("Test window index")
        ...
        loss_title = (
            "Train/validation total loss (weighted total loss for multi-head)"
            if len(val_losses) > 0
            else "Training total loss"
        )
        ...
        ax_price.plot(t_price, y_price.ravel(), label="True price", color="tab:blue", linewidth=1.1)
        ax_price.plot(
            t_price,
            p_price.ravel(),
        ...
        ax_price.set_title("Test set: true vs predicted (price)")
        ax_price.set_xlabel("Test window index")
        ...
        ax_res.set_title("Residual distribution for return prediction (y_true - y_pred)")
        ...
        ax_sc.set_title("True vs predicted returns")
        ax_sc.set_xlabel("True return")
        ...
        if use_multi and per_dir is not None and ydir_test is not None and pred_cls is not None:
            ...
            ax_dir.plot(true_dir, label="True direction (1=up, 0=down)", color="tab:blue", linewidth=1.0)
            ax_dir.plot(pred_cls, label="Predicted direction (threshold=0.5)", color="tab:orange", linewidth=1.0, alpha=0.8)
            ax_dir.set_title("Direction prediction comparison")
            ax_dir.set_xlabel("Test window index")
            ...
            if cm is not None:
                ...
                ax_cm.set_title("Direction confusion matrix")
            ...
            ax_prob.set_title("Direction head output probability distribution")
        ...
        ax_nav.set_title("Strategy NAV vs Buy & Hold")
        ax_nav.set_xlabel("Test window index")
