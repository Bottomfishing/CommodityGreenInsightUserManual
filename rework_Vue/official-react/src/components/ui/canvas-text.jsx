...
const DEFAULT_COLORS = ["#ff6b6b", "#4ecdc4", "#45b7d1", "#96ceb4", "#ffeaa7", "#dfe6e9"];
...
export function CanvasText({
  ...
  const updateColors = useCallback(() => {
    if (bgRef.current) {
      ...
      const nextBg = computed.backgroundColor;
      setBgColor((prev) => (prev === nextBg ? prev : nextBg));
    }
    const resolved = palette.map(resolveColor);
    const nextKey = resolved.join("|");
    setResolvedColors((prev) => {
      const prevKey = prev.join("|");
      return prevKey === nextKey ? prev : resolved;
    });
  }, [palette]);
