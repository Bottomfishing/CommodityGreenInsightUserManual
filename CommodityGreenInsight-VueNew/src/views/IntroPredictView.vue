<template>
  <div class="intro5-page">
    ...
    <main class="intro5-main">
      <section class="hero">
        <span class="kicker">GREEN FINTECH</span>
        <h1 class="hero-title">您可以选用您自己的金融模型或者我们的模型进行金融预测</h1>
        <p class="hero-sub">支持自定义模型接入与平台模型快速预测，统一输出价格、收益与风险信号</p>
        <div class="action-row">
          <button class="btn-primary" @click="goNext">继续</button>
          <button class="btn-ghost" @click="router.push({ name: 'IntroGlobe' })">返回上一页</button>
        </div>
      </section>
      ...
      <section class="showcase">
        ...
        >
          ...
          <p class="forecast-label">明日预测价格</p>
          <p class="forecast-value">$74.111</p>
          <p class="forecast-sub">相比最新价 $74.000 · 预测收益 0.001500</p>
        </article>
      </section>
    </main>
  </div>
...
function goNext() {
  router.push({ name: "IntroGreenForecast" });
}
...
.intro5-main {
  ...
  display: grid; grid-template-columns: minmax(0, 0.9fr) minmax(0, 1.1fr); gap: 22px; align-items: center; padding: 24px 0 28px;
}
...
.hero-title {
  margin: 0; font-size: clamp(34px, 4.1vw, 50px); font-weight: 800; letter-spacing: 0.02em; line-height: 1.14;
...
.forecast-card {
  ...
  padding: 26px 28px;
  background:
    radial-gradient(circle at 50% 50%, rgba(56, 189, 248, 0.12), rgba(2, 6, 23, 0.72) 62%),
...
.forecast-card::after {
  ...
  opacity: 0;
  animation: scanSweep 9.6s ease-in-out infinite;
  pointer-events: none;
  z-index: 2;
...
.forecast-value {
  ...
  font-weight: 800;
  color: #e0f2fe;
  text-shadow: 0 0 24px rgba(186, 230, 253, 0.22);
}
.forecast-sub {
  position: relative;
  margin: 10px 0 0;
...
@media (max-width: 980px) {
  .intro5-main { grid-template-columns: 1fr; align-items: start; }
  .forecast-label { font-size: 24px; }
  .forecast-sub { font-size: 20px; }
}
